...
class Config(object):
    class Wallabag(object):
        def __init__(self, debug = False):
            self.url = e.get('wallpod_wallabag_url')
            self.username = e.get('wallpod_wallabag_username')
            self.password = e.get('wallpod_wallabag_password')
            self.client_id = e.get('wallpod_wallabag_client_id')
            self.client_secret = e.get('wallpod_wallabag_client_secret')
            self.debug = debug
            return
    class Pocket(object):
        def __init__(self, debug = False):
            self.consumer_key = e.get('wallpod_pocket_consumer_key')
            self.access_token = e.get('wallpod_pocket_access_token')
            self.debug = debug
            return
    class Pod(object):
        def __init__(self, final_path = '', debug = False):
            self.url = path.join(e.get('wallpod_pod_url'),'')
            self.name = e.get('wallpod_pod_name','TTS podcast')
            self.author = e.get('wallpod_pod_author','TTS podcast author')
            self.image = e.get('wallpod_pod_image')
            if self.image and not 'http' in self.image:
                self.image = self.url + self.image
            self.description = e.get('wallpod_pod_description','TTS podcast description')
            self.language = e.get('wallpod_pod_language','en')
            self.ssh_server_path = e.get('wallpod_pod_ssh_server_path')
            if (self.ssh_server_path): self.ssh_server_path=path.join(self.ssh_server_path,'')
            self.ssh_server = e.get('wallpod_pod_ssh_server')
    ...
    class Speech(object):
        def __init__(self, temp_path = '', final_path = '', debug = False):
            ...
            self.engine = e.get('wallpod_engine','')
            self.eleven_api_key = e.get('wallpod_eleven_api_key')
            self.eleven_voice = e.get('wallpod_eleven_voice','Daniel')
            self.eleven_model = e.get('wallpod_eleven_model','eleven_monolingual_v1')
            self.openai_api_key = e.get('wallpod_openai_api_key')
            self.openai_voice = e.get('wallpod_openai_voice','onyx')
            self.openai_model = e.get('wallpod_openai_model','tts-1-hd')
            self.whisper_model = e.get('wallpod_whisper_model','collabora/whisperspeech:t2s-base-en+pl.model')
    ...
    def __init__(self,debug):
        load_dotenv()
        self.debug = e.get('wallpod_debug',debug)
        if self.debug: print(f'debug mode is on')
        self.max_length = int(e.get('wallpod_max_length',20000))
        self.max_articles = int(e.get('wallpod_max_articles',5))
        self.wallabag = self.Wallabag(debug = self.debug)
        self.pocket = self.Pocket(debug = self.debug)
        self.working_path = path.join(e.get('wallpod_working_path','./working'),'')
        self.temp_path = f'{self.working_path}temp/'
        self.final_path = f'{self.working_path}output/'
        self.pickle = f'{self.working_path}wallpod.pickle'
